...
export default function SentimentPanel({ positive, neutral, negative, keywords, loading, error }: SentimentProps) {
  return (
    <div className="bg-[#242424] rounded-2xl p-6 border border-[#2a2a2a]">
      <h3 className="text-lg font-semibold mb-4 text-white">🧠 市场反馈信号 <span className="text-xs text-[#00d4ff] font-normal ml-1">Claude AI</span></h3>
      {error ? (
        ...
        <>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {keywords.map((kw) => (
                <span key={kw} className="px-2 py-1 bg-[#00d4ff]/10 text-[#00d4ff] rounded-full text-xs">{kw}</span>
              ))}
            </div>
          </div>
        </>
